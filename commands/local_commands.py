...
class OpenAppCommand(BaseCommand):
    ...
    # Common Windows applications and their executable names/paths
    APPS = {
        "notepad": "notepad.exe",
        "calculator": "calc.exe",
        "paint": "mspaint.exe",
...
class CloseAppCommand(BaseCommand):
    ...
    def execute(self, user_input: str) -> Optional[str]:
        ...
        # Don't close if it's an exit command for AVA
            ...
            if app_name in user_input_lower:
                try:
                    if isinstance(process, list):
                        # Close multiple processes (like all browsers)
                        for proc in process:
...
class SystemControlCommand(BaseCommand):
    ...
    def execute(self, user_input: str) -> Optional[str]:
        ...
        # These are destructive operations, so be careful
        try:
            ...
            elif "lock" in user_input_lower:
                subprocess.run('rundll32.exe user32.dll,LockWorkStation', shell=True)
                return "Locking the computer."
            
            elif "log off" in user_input_lower or "logoff" in user_input_lower:
                return "For safety, please use the Start menu to log off."
